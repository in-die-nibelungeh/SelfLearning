...
double GetLse(mcon::Vector<double>& data, mcon::VectordBase& ref)
{
...
void test_interp()
{
    ...
    mcon::VectordBase& data   = matrix[0];
    mcon::VectordBase& coarse_index = matrix[3];
    ...
    for (uint k = 0; k < data.GetLength(); ++k )
    {
        ...
        if ( (k % every) == 0 )
        {
            ...
            coarse[k/every] = VALUE(static_cast<int>(k));
        }
    }
